...
class A2E(object):
    ...
    def get_expression_from_audio(self, path_audio, smooth=0):
        ...
        output = self.sess.run(self.output_a2e, feed_dict={self.input_a2e: input_audio})[0]
        output[output < 0] = 0
...
class AudioHandler(object):
    ...
    def build_model(self):
        ...
        # tensor
        self.input_ds = self.graph.get_tensor_by_name('deepspeech/input_node:0') 
        self.seq_length_ds = self.graph.get_tensor_by_name('deepspeech/input_lengths:0')
    ...
    def process_audio(self, path_audio):
        ...
        audio_inter = self.interpolate_feature(audio_ds_logit, 50, self.fps, n_frames)
    ...
    def convert_mfcc(self, audio):
        ...
        empty_context = np.zeros((self.n_context, self.n_mfcc), dtype=audio_mfcc.dtype)
        ...
        audio_mfcc_window = (audio_mfcc_window - np.mean(audio_mfcc_window)) / np.std(audio_mfcc_window)
    ...
    def interpolate_feature(self, input_features, input_rate, output_rate, n_frames):
        ...
        for feat in range(n_features):
            output_features[:, feat] = np.interp(output_timestamps, input_timestamps, input_features[:,0][:, feat])
